...
test("canvas stays pointer-inert and renderer owns the only runtime pose bridge", async () => {
  ...
  assert.match(material, /brassLight = vec3\(0\.440, 0\.335, 0\.198\)/);
  assert.match(material, /brassMid = vec3\(0\.370, 0\.270, 0\.154\)/);
  assert.match(material, /brassDark = vec3\(0\.180, 0\.116, 0\.063\)/);
  ...
  assert.match(radial, /#kinetic-instrument\[data-material-prototype="roughness"\] \.m2-solar-brass-response\s*\{[^}]*opacity:\s*\.025;/);
