...
test("canvas stays pointer-inert and renderer owns the only runtime pose bridge", async () => {
  ...
  assert.equal((grainPattern.match(/<circle /g) ?? []).length, 132);
  assert.match(grainPattern, /fill="#eef1ef" fill-opacity="\.18"/);
  assert.match(grainPattern, /fill="#020303" fill-opacity="\.20"/);
  assert.match(grainPattern, /fill="#c9cecb" fill-opacity="\.14"/);
  assert.match(grainPattern, /fill="#0a0c0c" fill-opacity="\.16"/);
  ...
  assert.match(material, /fineGrain = fieldCentered \* 0\.030/);
  assert.match(material, /directionalSheen = specular \* environmentLift \* 1\.45/);
  assert.match(material, /overlayAlpha = edgeMask \* clamp\(directionalSheen \+ fineGrain, 0\.006, 0\.070\)/);
