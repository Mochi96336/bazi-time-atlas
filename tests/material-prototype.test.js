...
test("canvas stays pointer-inert and renderer owns the only runtime pose bridge", async () => {
  ...
  assert.match(material, /roughness = clamp\(0\.810 \+ fieldCentered \* 0\.095, 0\.768, 0\.850\)/);
  assert.match(material, /fieldDx = \([\s\S]*?fieldDy = \(/);
  assert.match(material, /slopeLocal = vec2\(fieldDx, fieldDy\) \* 0\.46/);
  ...
  assert.match(material, /microAlpha = clamp\(abs\(microLightDelta\) \* 0\.42 \+ abs\(fieldCentered\) \* 0\.014, 0\.0, 0\.036\)/);
  assert.match(material, /overlayAlpha = edgeMask \* clamp\(specularAlpha \+ microAlpha, 0\.0, 0\.046\)/);
