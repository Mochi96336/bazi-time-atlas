...
test("Tools disclosure uses current product capabilities without duplicating a Tools prefix in the edge rail", () => {
  ...
  assert.match(view, /顯示找時間、分類與太陽時間等工具/);
  assert.match(view, /需要找時間、分類或太陽時間比較時再展開/);
  assert.doesNotMatch(view, /固定視角/);
  assert.match(view, /收起工具；目前設定會保留/);
  assert.doesNotMatch(css, /content:\s*"工具 · "/);
  assert.doesNotMatch(view, /尺度、圖層|圖層顯示/);
});

test("reference-frame engine control remains installed for regression ownership", () => {
  assert.match(referenceControls, /select\.id = "reference-frame-select"/);
  assert.match(referenceControls, /world\.value = "world";[\s\S]*world\.textContent = "不固定"/);
  assert.match(referenceControls, /select\.addEventListener\("change"/);
});
