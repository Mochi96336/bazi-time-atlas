...
test("Ganzhi active sectors stay subordinate to the Selected Instant read-head", () => {
  assert.match(
    hierarchy,
    /#hour-track \.cycle-sector\.is-active,[\s\S]*?#year-track \.cycle-sector\.is-active\s*\{[\s\S]*?stroke:\s*color-mix\(in srgb, var\(--cursor\) 30%, transparent\);[\s\S]*?stroke-width:\s*1\.05;/
  );
