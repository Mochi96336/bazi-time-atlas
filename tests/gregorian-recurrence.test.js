...
import {
  ...
  findGlobalGregorianYearDayPeriod,
  gregorianOrdinal,
  recurrenceState
...
test("2026-09-13 first repeats Year+Day phases after 1980 years", () => {
  const local = findFirstLocalYearDayRecurrence(BASE);
  assert.equal(local.deltaYears, 1980);
  assert.equal(local.phases.year, 0);
...
test("canonical milestones show why 24000 years is the first global closure", () => {
  ...
  assert.deepEqual(at400.phases, { year: 40, gregorian: 0, day: 57 });

  const at1200 = recurrenceState(BASE, 1200);
  assert.deepEqual(at1200.phases, { year: 0, gregorian: 0, day: 51 });

  const at8000 = recurrenceState(BASE, 8000);
  assert.deepEqual(at8000.phases, { year: 20, gregorian: 0, day: 0 });

  const at24000 = recurrenceState(BASE, GLOBAL_GREGORIAN_YEAR_DAY_PERIOD);
  assert.deepEqual(at24000.phases, { year: 0, gregorian: 0, day: 0 });
  assert.equal(at24000.dayDelta, 8_765_820);
});

test("global Gregorian + Year + Day period search returns 24000 years", () => {
  const period = findGlobalGregorianYearDayPeriod();
