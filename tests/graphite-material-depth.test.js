...
test("wheel surface depth stays ring-local while visible SVG stipple stays absent", () => {
  ...
  assert.match(atlasHtml, /id="m2-rim-light-stroke"[\s\S]*?stop-opacity="\.24"[\s\S]*?stop-opacity="\.030"/);
  assert.match(atlasHtml, /id="m2-rim-shadow-stroke"[\s\S]*?stop-opacity="\.58"/);
  assert.match(radialHierarchy, /\.m2-ring-rim-light \{[\s\S]*?stroke-width:\s*1\.15;/);
  ...
  assert.match(radialHierarchy, /#m2-hour-surface \{[^}]*--m2-light-alpha:\s*\.260;[^}]*--m2-shoulder-alpha:\s*\.180;/);
  assert.match(radialHierarchy, /#m2-year-surface \{[^}]*--m2-light-alpha:\s*\.340;[^}]*--m2-shoulder-alpha:\s*\.235;/);
