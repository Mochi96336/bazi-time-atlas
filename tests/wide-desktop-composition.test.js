...
test("Visual gate owns a real 2047x1038 browser probe and PNG evidence", () => {
  ...
  assert.match(fixture, /scaleVisible:\s*String\(displayVisibleCount\(win, scaleButtons\)\)/);
  assert.match(fixture, /ringToggleVisible:\s*String\(displayVisibleCount\(win, ringToggles\)\)/);
  ...
  assert.match(browserGate, /evidenceLeft < width - 410/);
