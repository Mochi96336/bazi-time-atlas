...
const FRAGMENT_SHADER = [
  ...
  "  float wx = (solarFbm(p * 1.50, 51.0) - 0.5) * 1.05;",
  "  float wy = (solarFbm(p * 1.42 + vec2(17.0, -9.0), 73.0) - 0.5) * 0.92;",
  "  vec2 warped = p + vec2(wx, wy);",
  "  float low = solarFbm(warped * 1.72, 11.0);",
  "  float mid = solarFbm(vec2(warped.x * 3.20, warped.y * 2.98) + vec2(3.7, -2.1), 29.0);",
  "  float fine = solarFbm(warped * 5.20 + vec2(-5.2, 8.4), 101.0);",
  "  float combined = low * 0.36 + mid * 0.50 + fine * 0.14;",
  "  float oxidePatch = smoothstep(0.52, 0.68, combined);",
  "  float deep = smoothstep(0.65, 0.80, solarFbm(p * vec2(2.30, 2.00) + vec2(-12.0, 7.0), 149.0));",
  "  float cool = smoothstep(0.69, 0.84, solarValueNoise(p * vec2(1.85, 1.72) + vec2(5.0, 13.0), 211.0));",
  ...
  "  vec3 brassLight = vec3(0.440, 0.335, 0.198);",
  "  vec3 brassMid = vec3(0.370, 0.270, 0.154);",
  "  vec3 brassDark = vec3(0.180, 0.116, 0.063);",
  ...
  "  return mix(body, vec3(0.961, 0.878, 0.710), warmCatch * 0.015);",
  "}",
  "",
  ...
  "  float oxideStrength = clamp(aging.x * 0.18 + aging.y * 0.09 + aging.z * 0.012, 0.0, 0.26);",
