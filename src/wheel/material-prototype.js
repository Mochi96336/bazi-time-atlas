...
const FRAGMENT_SHADER = [
  ...
  "  float wx = (solarFbm(p * 0.60, 51.0) - 0.5) * 2.1;",
  "  float wy = (solarFbm(p * 0.55 + vec2(17.0, -9.0), 73.0) - 0.5) * 1.8;",
  "  vec2 warped = p + vec2(wx, wy);",
  "  float low = solarFbm(warped * 0.58, 11.0);",
  "  float mid = solarFbm(vec2(warped.x * 1.55, warped.y * 1.45) + vec2(3.7, -2.1), 29.0);",
  "  float fine = solarFbm(warped * 4.20 + vec2(-5.2, 8.4), 101.0);",
  "  float combined = low * 0.63 + mid * 0.30 + fine * 0.07;",
  "  float oxidePatch = smoothstep(0.41, 0.64, combined);",
  "  float deep = smoothstep(0.57, 0.75, solarFbm(p * vec2(1.05, 0.92) + vec2(-12.0, 7.0), 149.0));",
  "  float cool = smoothstep(0.69, 0.84, solarValueNoise(p * vec2(0.82, 0.78) + vec2(5.0, 13.0), 211.0));",
  ...
  "  float scratchLength = mix(8.0 + shortBias * 135.0, 25.0 + r3 * 170.0, handling) * demoScale;",
  ...
  "  float bend = bendWindow * (bendRandom - 0.5) * mix(10.0, 16.0, handling) * demoScale;",
  ...
  "  vec3 brassLight = vec3(0.773, 0.608, 0.376);",
  "  vec3 brassMid = vec3(0.596, 0.443, 0.259);",
  "  vec3 brassDark = vec3(0.212, 0.137, 0.075);",
  ...
  "  return mix(body, vec3(0.961, 0.878, 0.710), warmCatch * 0.08);",
  "}",
  "",
  ...
  "  float oxideStrength = clamp(aging.x * 0.44 + aging.y * 0.24 + aging.z * 0.04, 0.0, 0.62);",
