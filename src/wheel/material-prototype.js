...
const FRAGMENT_SHADER = [
  ...
  "  float wx = (solarFbm(p * 0.78, 51.0) - 0.5) * 1.82;",
  "  float wy = (solarFbm(p * 0.72 + vec2(17.0, -9.0), 73.0) - 0.5) * 1.56;",
  "  vec2 warped = p + vec2(wx, wy);",
  "  float low = solarFbm(warped * 0.75, 11.0);",
  "  float mid = solarFbm(vec2(warped.x * 1.86, warped.y * 1.74) + vec2(3.7, -2.1), 29.0);",
  "  float fine = solarFbm(warped * 4.40 + vec2(-5.2, 8.4), 101.0);",
  "  float combined = low * 0.63 + mid * 0.30 + fine * 0.07;",
  "  float oxidePatch = smoothstep(0.46, 0.65, combined);",
  "  float deep = smoothstep(0.60, 0.76, solarFbm(p * vec2(1.28, 1.12) + vec2(-12.0, 7.0), 149.0));",
  "  float cool = smoothstep(0.69, 0.84, solarValueNoise(p * vec2(1.02, 0.97) + vec2(5.0, 13.0), 211.0));",
  ...
  "  float scratchLength = mix(8.0 + shortBias * 135.0, 25.0 + r3 * 170.0, handling) * demoScale * 0.82;",
  ...
  "  float bend = bendWindow * (bendRandom - 0.5) * mix(10.0, 16.0, handling) * demoScale * 0.87;",
  ...
  "  vec3 brassLight = vec3(0.696, 0.547, 0.338);",
  "  vec3 brassMid = vec3(0.542, 0.403, 0.236);",
  "  vec3 brassDark = vec3(0.206, 0.133, 0.073);",
  ...
  "  return mix(body, vec3(0.961, 0.878, 0.710), warmCatch * 0.055);",
  "}",
  "",
  ...
  "  float oxideStrength = clamp(aging.x * 0.31 + aging.y * 0.16 + aging.z * 0.025, 0.0, 0.45);",
  ...
  "  float primaryStrength = abs(primaryScratch.x) * 0.028 * fineAttenuation;",
  "  float handlingStrength = abs(handlingScratch.x) * 0.018 * fineAttenuation;",
