...
const FRAGMENT_SHADER = [
  ...
  "const float SURFACE_PERIOD_LARGE = 156.0;",
  "const float SURFACE_PERIOD_MEDIUM = 58.0;",
  "const float SURFACE_PERIOD_FINE = 23.0;",
  "const float SURFACE_SAMPLE_STEP = 1.35;",
  "",
  ...
  "  vec2 mediumPoint = rotation(0.31) * localPoint;",
  "  vec2 finePoint = rotation(-0.47) * localPoint;",
  ...
  "  return large * 0.34 + medium * 0.43 + fine * 0.23;",
  "}",
  "",
  "void main() {",
  ...
  "  float roughness = clamp(0.805 + fieldCentered * 0.11, 0.760, 0.850);",
  "",
  "  float fieldDx = (",
  ...
  "    - surfaceField(localPoint - vec2(0.0, SURFACE_SAMPLE_STEP))",
  "  ) / (2.0 * SURFACE_SAMPLE_STEP);",
  "  vec2 slopeLocal = vec2(fieldDx, fieldDy) * 0.72;",
  ...
  "  float microLightDelta = clamp(dot(microNormal, lightDirection) - baseLight, -0.075, 0.075);",
  ...
  "  float specularAlpha = clamp(specular * environmentResponse * 0.50, 0.0, 0.013);",
  "  float microAlpha = clamp(abs(microLightDelta) * 0.46 + abs(fieldCentered) * 0.016, 0.0, 0.038);",
  "  vec3 microTint = microLightDelta >= 0.0 ? microLightTint : microDarkTint;",
  "  float overlayAlpha = edgeMask * clamp(specularAlpha + microAlpha, 0.0, 0.051);",
