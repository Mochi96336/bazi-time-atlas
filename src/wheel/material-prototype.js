...
const FRAGMENT_SHADER = [
  ...
  "}",
  "",
  "vec4 scratchFromCell(vec2 uv, float cell, float cellSize, float handling, float circumference, float phase) {",
  ...
  "}",
  "",
  "vec4 scratchLayer(vec2 localPoint, float handling, float phase) {",
  ...
  "    vec4 candidate = scratchFromCell(",
  "      uv,",
  ...
  "}",
  "",
  "vec4 primaryScratchField(vec2 localPoint) {",
  "  vec4 first = scratchLayer(localPoint, 0.0, 0.0);",
  "  vec4 second = scratchLayer(localPoint, 0.0, 1.0);",
  "  return second.y > first.y ? second : first;",
  "}",
  "",
  "vec4 handlingScratchField(vec2 localPoint) {",
  "  return scratchLayer(localPoint, 1.0, 2.0);",
  "}",
  "",
  "vec3 brassBody(vec2 worldPoint) {",
  "  vec3 brassLight = vec3(0.328, 0.282, 0.230);",
  "  vec3 brassMid = vec3(0.286, 0.241, 0.198);",
  "  vec3 brassDark = vec3(0.138, 0.105, 0.085);",
  ...
  "  vec4 primaryScratch = u_material_probe == 2 ? vec4(0.0) : primaryScratchField(localPoint);",
  "  vec4 handlingScratch = u_material_probe == 2 ? vec4(0.0) : handlingScratchField(localPoint + vec2(19.0, -11.0));",
  "",
  "  vec3 body = brassBody(worldPoint);",
  "  vec3 oxideWarm = vec3(0.245, 0.180, 0.128);",
  "  vec3 oxideCool = vec3(0.267, 0.292, 0.284);",
  ...
  "  vec3 scratchLight = vec3(0.880, 0.852, 0.775);",
  "  vec3 scratchDark = vec3(0.125, 0.091, 0.067);",
  ...
  "",
  "  // H2.1: the same deterministic scratch geometries now carry shallow groove",
  "  // side normals. Material rotates with its ring; the world light stays fixed.",
  "  // No extra noise field, wider scratches or global brass brightness shift.",
  "  if (u_material_probe != 7 && primaryScratch.y + handlingScratch.y > 0.001) {",
  "    vec2 radialLocal = normalize(localPoint);",
  "    vec2 tangentLocal = vec2(-radialLocal.y, radialLocal.x);",
  "    vec2 grooveLocal = tangentLocal * (primaryScratch.z * 0.32 + handlingScratch.z * 0.10)",
  "      + radialLocal * (primaryScratch.w * 0.32 + handlingScratch.w * 0.10);",
  "    vec2 grooveWorld = rotation(-ringRotation) * grooveLocal;",
  "    vec3 scratchNormal = normalize(vec3(-grooveWorld, 1.0));",
  "    vec3 fixedLight = normalize(vec3(-0.42, -0.56, 0.714));",
  "    vec3 halfVector = normalize(fixedLight + vec3(0.0, 0.0, 1.0));",
  "    float tiltedLight = dot(scratchNormal, fixedLight) - fixedLight.z;",
  "    float tiltedSpecular = pow(max(dot(scratchNormal, halfVector), 0.0), 12.0)",
  "      - pow(halfVector.z, 12.0);",
  "    float localReflection = clamp(tiltedLight * 0.42 + tiltedSpecular * 0.12, -0.055, 0.055)",
  "      * fineAttenuation;",
  "    body = clamp(body + vec3(0.60, 0.53, 0.44) * localReflection, 0.0, 1.0);",
  "  }",
  "",
  ...
  "}",
  "",
  "float zodiacNebulaField(vec2 localPoint) {",
  "  vec2 p = vec2(localPoint.x / 128.0, localPoint.y / 112.0);",
  "  float wx = (solarFbm(p * 1.55 + vec2(4.2, -7.1), 419.0) - 0.5) * 0.42;",
  "  float wy = (solarFbm(p * 1.45 + vec2(-8.6, 5.3), 431.0) - 0.5) * 0.38;",
  "  vec2 warped = p + vec2(wx, wy);",
  "  float low = solarFbm(warped * 2.25 + vec2(2.7, -1.9), 443.0);",
  "  float mid = solarFbm(vec2(warped.x * 4.40, warped.y * 3.90) + vec2(-3.1, 6.4), 461.0);",
  "  float fine = solarFbm(warped * 7.20 + vec2(8.3, 2.6), 487.0);",
  "  return clamp(low * 0.24 + mid * 0.50 + fine * 0.26, 0.0, 1.0);",
  "}",
  "",
  ...
  "  vec3 nebulaDarkTint = vec3(0.005, 0.009, 0.015);",
  "  float nebulaAlpha = nebulaLightAlpha + nebulaDarkAlpha;",
  "  float overlayAlpha = edgeMask * clamp(specularAlpha + microAlpha + nebulaAlpha, 0.0, 0.110);",
  "  vec3 overlayColor = reflectionTint * specularAlpha + microTint * microAlpha",
  "    + nebulaLightTint * nebulaLightAlpha + nebulaDarkTint * nebulaDarkAlpha;",
