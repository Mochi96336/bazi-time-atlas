...
const FRAGMENT_SHADER = [
  ...
  "  float roughness = clamp(0.810 + fieldCentered * 0.095, 0.768, 0.850);",
  "",
  "  float fieldDx = (",
  ...
  "    - surfaceField(localPoint - vec2(0.0, SURFACE_SAMPLE_STEP))",
  "  ) / (2.0 * SURFACE_SAMPLE_STEP);",
  "  vec2 slopeLocal = vec2(fieldDx, fieldDy) * 0.46;",
  ...
  "  float microAlpha = clamp(abs(microLightDelta) * 0.42 + abs(fieldCentered) * 0.014, 0.0, 0.036);",
  "  vec3 microTint = microLightDelta >= 0.0 ? microLightTint : microDarkTint;",
  "  float overlayAlpha = edgeMask * clamp(specularAlpha + microAlpha, 0.0, 0.046);",
