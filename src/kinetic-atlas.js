...
const ringSpecs = {
  year: { group: document.querySelector("#year-track"), inner: 258, outer: 332, className: "year-sector" },
  month: { group: document.querySelector("#month-track"), inner: 332, outer: 406, className: "month-sector" },
  day: { group: document.querySelector("#day-track"), inner: 406, outer: 480, className: "day-sector" }
};
...
function renderGuides() {
  ...
  [258, 332, 406, 480, 570, 680].forEach(radius => {
...
function renderSolarRing() {
  solarTerms.forEach((term, index) => {
    ...
    const path = svgEl("path", {
      d: annularSectorPath(480, 570, start + .15, end - .15),
    ...
    const markInner = polar(480, term.longitude);
    const markOuter = polar(term.kind === "jie" ? 570 : 558, term.longitude);
    svgEl("line", {
      x1: markInner.x,
      y1: markInner.y,
      x2: markOuter.x,
    ...
    const labelPoint = polar(term.kind === "jie" ? 538 : 526, term.longitude + 7.5);
...
function renderZodiacRing() {
  zodiacSigns.forEach((sign, index) => {
    const path = svgEl("path", {
      d: annularSectorPath(570, 680, sign.start + .15, sign.end - .15),
      class: "zodiac-sector",
...
function renderCursor() {
  ...
  const inner = polar(246, CURSOR_ANGLE);
  const outer = polar(692, CURSOR_ANGLE);
  ...
  const cap = polar(701, CURSOR_ANGLE);
  svgEl("path", {
    d: `M ${cap.x - 6} ${cap.y - 1} L ${cap.x + 6} ${cap.y - 1} L ${cap.x} ${cap.y + 10} Z`,
    class: "cursor-cap"
  }, cursorLayer);
  const label = polar(720, CURSOR_ANGLE);
