...
if (attr(probe, "data-hidden-pressed") !== "false" || attr(probe, "data-hidden-track") !== "true" || attr(probe, "data-hidden-display") !== "none") {
  throw new Error(`visibility: Day toggle did not hide the track cleanly: ${url}`);
}
...
if (![instantBefore, instantAfterStep, modelBefore, modelAfterStep, hiddenDragBefore, hiddenDragAfter, dayOffset].every(Number.isFinite)) {
...
if (attr(probe, "data-restored-pressed") !== "true" || attr(probe, "data-restored-track") !== "false" || attr(probe, "data-restored-display") === "none") {
...
console.log(`[ring-visibility] PASS true 390px hide/update/isolate/restore; Day model ${modelBefore}->${modelAfterStep}, master +${DAY_MS}ms: ${url}`);
