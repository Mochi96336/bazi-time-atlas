...
typedef struct {
  ...
  double max_jerk_bound_au_per_day3;
} body_bound;
...
static double cheb_third_derivative_basis_bound(int n) {
  if (n < 3) return 0.0;
  const double n2 = (double)n * (double)n;
  return n2 * (n2 - 1.0) * (n2 - 4.0) / 15.0;
}
...
static void segment_vector_bounds(
  ...
  for (int coord = 0; coord < 3; coord++) {
    ...
    for (int n = 1; n < pdp->neval; n++) {
      ...
      if (n >= 3)
        jcoord[coord] += abs_c * cheb_third_derivative_basis_bound(n) * dx_dt * dx_dt * dx_dt;
    }
  }
  ...
  *jerk_bound = hypot(hypot(jcoord[0], jcoord[1]), jcoord[2]);
}
...
static void inspect_loaded_segment(body_bound *body) {
  ...
  double pbound = 0, vbound = 0, abound = 0, jbound = 0;
  segment_vector_bounds(pdp, &pbound, &vbound, &abound, &jbound);
  ...
  body->max_jerk_bound_au_per_day3 = max2(body->max_jerk_bound_au_per_day3, jbound);
}
...
static void print_body_json(const body_bound *body) {
  ...
  printf("      \"maxJerkBoundAuPerDayCubed\": %.17g\n", body->max_jerk_bound_au_per_day3);
  printf("    }");
}

int main(int argc, char **argv) {
  if (argc != 2) die("usage: research-swiss-aberration-precessed-ra-samples-4006 <ephemeris-dir>");
  ...
  /* Exact Swiss geocentric Sun path with light-time and annual aberration
   * enabled, nutation disabled, and equatorial mean-of-date output. */
  const int aberration_flags = SEFLG_SWIEPH | SEFLG_SPEED | SEFLG_XYZ |
      SEFLG_EQUATORIAL | SEFLG_NOGDEFL | SEFLG_NONUT;

  double min_sampled_aberration_precessed_xy_au = HUGE_VAL;
  long min_index = -1;
  double min_jd_et = 0;
  ...
  for (long i = 0; i <= intervals; i++) {
    ...
    calc_checked(jd_et, SE_SUN, aberration_flags, app, serr);
    const double rxy = hypot(app[0], app[1]);
    if (rxy < min_sampled_aberration_precessed_xy_au) {
      min_sampled_aberration_precessed_xy_au = rxy;
      min_index = i;
      min_jd_et = jd_et;
    }
  }
  ...
  printf("{\n");
  printf("  \"targetYear\": %d,\n", TARGET_YEAR);
  printf("  \"method\": \"swieph-two-pass-light-time-plus-annual-aberration-plus-vondrak-et-grid-v1\",\n");
  ...
  printf("  \"minSampledAberrationPrecessedXyAu\": %.17g,\n", min_sampled_aberration_precessed_xy_au);
