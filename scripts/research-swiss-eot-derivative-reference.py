...
def main() -> None:
    ...
    for sample in samples:
        ...
        label = f"{year:04d}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:{second:06.3f}"

        # Fail closed if the required long-range Swiss ephemeris files are not
        # actually active. As in the corrected #160 capture, both Sun and Moon
        # are checked before swe_time_equ() so missing deep-time files cannot
        # silently fall back to Moshier.
