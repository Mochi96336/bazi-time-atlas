...
if (before.resetHidden !== "true") {
  throw new Error(`baseline should keep retired Compare Reset absent: ${JSON.stringify(before)}: ${url}`);
}
...
function assertDetachedControls(phase, value) {
  const finite = [
    value.toolbarLeft,
    value.toolbarRight,
    value.toolbarTop,
  ...
  if (!finite) throw new Error(`${phase} retired Compare control geometry is not finite: ${JSON.stringify(value)}: ${url}`);
  if (!(value.resetHidden === "false" && value.resetWidth > 0 && value.resetHeight > 0)) {
    throw new Error(`${phase} should expose one real Reset control after detachment: ${JSON.stringify(value)}: ${url}`);
  }
  if (value.actionToolbarOverflow !== "false") {
    throw new Error(`${phase} retired Compare controls escape the toolbar bounds: ${JSON.stringify(value)}: ${url}`);
  }
}
...
assertDetachedControls("detached", detached);
...
assertDetachedControls("allDetached", allDetached);
...
assertDetachedControls("combined", combined);
...
console.log(
  ...
  `Day offset 0°→${detached.manualOffset.toFixed(1)}°; retired Compare Reset remained bounded; all five detached; ` +
  `worst status ${allDetached.statusWidth.toFixed(1)}×${allDetached.statusHeight.toFixed(1)}px; ` +
