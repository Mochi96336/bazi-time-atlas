...
if (toolbarTop < instrumentTop || toolbarBottom > instrumentTop + 70) {
  throw new Error(`wide desktop: primary actions left the compact top chrome (${toolbarTop}-${toolbarBottom}): ${page.url}`);
}
if (legendTop < instrumentTop || legendBottom > instrumentTop + 95) {
  throw new Error(`wide desktop: reference control left the upper-left edge zone (${legendTop}-${legendBottom}): ${page.url}`);
}
...
if (evidenceRight < width - 2 || evidenceRight > width + 1 || evidenceLeft < width - 410 || evidenceTop < 75 || evidenceBottom >= readoutTop) {
...
if (closeTop < instrumentTop || closeBottom > instrumentTop + 95 || closeRight < width - 60) {
  throw new Error(`wide desktop: Done action left the compact upper-right edge (${closeTop}-${closeBottom}@${closeRight}): ${page.url}`);
}
...
console.log(
  ...
  `left=${timelineLeft}-${solarRight}, right=${evidenceLeft}-${evidenceRight}, scrollHeight=${scrollHeight}, ` +
