...
const TARGET_YEAR = 2426;
const API_URL = "https://ssd.jpl.nasa.gov/api/horizons.api";
const OUTPUT = `jpl-horizons-${TARGET_YEAR}-evidence.json`;
...
function shouXingCandidateTtJulianDay(year, longitudeDegrees) {
  ...
  const marchEquinoxGuess = SolarTime.fromYmdHms(year, 3, 20, 12, 0, 0).getJulianDay().getDay();
...
async function horizonsTable({ startJd, stopJd, stepSize }) {
  ...
  const rows = table.trim().split(/\r?\n/).filter(Boolean).map(line => {
    ...
    return { calendarTt:fields[0], jdTt:Number(fields[1]), longitudeDegrees:Number(fields[4]) };
  });
...
async function solveHorizonsCrossing(candidateJd, targetDegrees) {
  ...
  const refinedRows = await horizonsTable({
    startJd:coarseJd - 2 / 1440,
    stopJd:coarseJd + 2 / 1440,
    stepSize:"240"
  });
...
for (const term of solarTerms) {
  ...
  evidenceTerms.push({
    ...
    epochErrorSeconds:Number(errorSeconds.toFixed(6))
  });
  console.error(`${term.name.padEnd(2)} ${String(term.longitude).padStart(3)}° error ${errorSeconds.toFixed(6)} s`);
}
const absoluteErrors = evidenceTerms.map(term => Math.abs(term.epochErrorSeconds));
const evidence = {
  schemaVersion:1,
  id:`jpl-horizons-de441-${TARGET_YEAR}-24-term`,
  ...
  sampledYears:[TARGET_YEAR],
  samplesByYear:{ [TARGET_YEAR]:evidenceTerms.length },
  crossings:evidenceTerms.length,
  maxEpochErrorSeconds:Number(Math.max(...absoluteErrors).toFixed(6)),
