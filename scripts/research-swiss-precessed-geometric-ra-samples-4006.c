...
static int calc_checked(double jd_et, int planet, int flags, double *x, char *serr) {
...
int main(int argc, char **argv) {
  if (argc != 2) die("usage: research-swiss-precessed-geometric-ra-samples-4006 <ephemeris-dir>");
  ...
  double min_sampled_precessed_xy_au = HUGE_VAL;
  long min_index = -1;
  double min_jd_et = 0;
  ...
  for (long i = 0; i <= intervals; i++) {
    ...
    double rel[3] = {xs[0] - xe[0], xs[1] - xe[1], xs[2] - xe[2]};
    if (swi_precess(rel, jd_et, 0, J2000_TO_J) != OK) die("swi_precess failed");
    const double rxy = hypot(rel[0], rel[1]);
    if (rxy < min_sampled_precessed_xy_au) {
      min_sampled_precessed_xy_au = rxy;
      min_index = i;
      min_jd_et = jd_et;
    }
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (b->first_tseg0 > start_et + COVER_TOLERANCE_DAYS ||
        b->last_tseg1 < end_et - COVER_TOLERANCE_DAYS) {
      die("SWIEPH segment coverage does not span year-4006 ET domain");
    }
  ...
  printf("{\n");
  printf("  \"targetYear\": %d,\n", TARGET_YEAR);
  printf("  \"method\": \"swieph-j2000-geometric-plus-vondrak-precession-et-grid-v1\",\n");
  ...
  printf("  \"minSampledPrecessedXyAu\": %.17g,\n", min_sampled_precessed_xy_au);
  printf("  \"minSampleIndex\": %ld,\n", min_index);
  printf("  \"minSampleEtJd\": %.15f,\n", min_jd_et);
  printf("  \"interpretation\": {\n");
  printf("    \"uniformEtGrid\": true,\n");
  printf("    \"precessionAppliedByPinnedSwissSwiPrecess\": true,\n");
