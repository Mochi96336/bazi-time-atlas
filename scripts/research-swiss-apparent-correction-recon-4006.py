...
This is reconnaissance only. Five-minute correction curvature measurements are
used to prioritize later source-derived proofs; they are never promoted into
continuous hard bounds.
"""
...
def main() -> None:
    ...
    initial_ras = [float(state[name]["prevRa"]) for name, _, _ in LAYERS]
    ...
    ranked = sorted(
        (
            {
                "correction": name,
                "observedMaxAbsSecondDifferenceDegPerDaySquared": float(
                    item["worstObserved"]["correctionSecondDifferenceDegPerDaySquared"]["abs"]
                ),
            }
            for name, item in corrections_out.items()
        ),
        key=lambda x: x["observedMaxAbsSecondDifferenceDegPerDaySquared"],
        reverse=True,
    )
    ...
    manifest = {
        ...
        "rankedByObservedCorrectionCurvature": ranked,
        "interpretation": {
            "empiricalOnly": True,
